 ...
 * Click the "RCP Membership" tab to assign an RCP subscription level to the product. This subscription level
 * will be granted to any users who purchase this product.
 *
 * NOTES:
 *
 * This integration does not work with renewals, so it's best used with subscription levels that have an unlimited
 * duration. If used with a subscription level that has a fixed duration (not unlimited), the user's expiration
 ...
 * if they purchased a product that's assigned a subscription level.
 *
...
function ag_rcp_woo_payment_complete( $order_id, $order = false ) {
	...
	foreach ( $items as $item ) {
		...
		$rcp_subscription_level_id = get_post_meta( $product_id, '_rcp_woo_subscription_level_id', true );
		...
		rcp_add_user_to_subscription( $user->ID, $subscription_args );

		// Flag this membership as coming from WooCommerce.
		rcp_add_member_note( $user->ID, sprintf( __( 'Membership added via WooCommerce order #%d' ), $order_id ) );
